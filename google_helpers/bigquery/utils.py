        ...
        if len(values) > 0:
            ...
            # by checking for the proper attr_name in the optional continuous_numerics list
                ...
                if len(values) == 2 and type(values[0]) is not list:
                    ...
                    if not all_pairs:
                        logger.error(
                            "[ERROR] While parsing attribute {}, calculated to be a numeric range filter, found an unparseable value:".format(
                                attr_name))
                        logger.error("[ERROR] {}".format(values))
                        continue
                btw_filter_strings = []
                ...
                if value_op == 'AND':
                    val_scalars = ["{}{} = {}".format(field_prefix or '', attr_name,
                                                      "'{}'".format(x) if parameter_type == "STRING" else x) for x in
                                   values]
                    ...
                    filter_string += "{}{} IN ({})".format('' if not field_prefix else field_prefix, attr_name,
                                                           val_list)
